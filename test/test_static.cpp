...
TEST_F(ZeroStaticArrayFixture, AccessElement2) {
  ...
  ASSERT_FALSE(math::all_equal(matrix(0), ans));
}

class MultiDimensional : public ::testing::Test {
protected:
  math::StaticVectori<2> vector{{1, 2}};
  math::StaticArrayi<2, 2> matrix{{{1, 2}, {3, 4}}};
  math::StaticArrayi<2, 2, 2> tensor{{{{1, 2}, {3, 4}}, {{5, 6}, {7, 8}}}};
};
...
TEST(ARange, Vector) {
  ...
  math::StaticVector<uint64_t, 4> answer = {0, 1, 2, 3};
  ASSERT_TRUE(math::all_equal(range, answer));
}

TEST_F(MultiDimensional, IndexingVector) {
  math::StaticVector<uint64_t, 1> indices = {1};
...
TEST_F(MultiDimensional, IndexingMatrix) {
  math::StaticVector<uint64_t, 1> indices = {1};
  math::StaticArrayi<1, 2> answer = {{3, 4}};
